...
class GoogleDriveService {
  ...
  static List<Object> _mapRowDataToValueList(Map<String, dynamic> rowData) {
    ...
    // Formulas are escaped using a leading quote
    Object escapeFormula(Object val) {
      if (val is String && (val.startsWith('=') || val.startsWith('+') || val.startsWith('-') || val.startsWith('@'))) {
        return "'$val";
      }
      return val;
    }
    ...
    return [
      escapeFormula(dpi),
      escapeFormula(nombre),
      escapeFormula(proyecto),
      escapeFormula(fecha),
      escapeFormula(hora),
      escapeFormula(usuario),
      escapeFormula(tipo)
    ];
  }
  ...
  static Future<bool> appendAttendanceRow(
    ...
    try {
      ...
      final valueRange = sheets.ValueRange()
        ..values = [
          _mapRowDataToValueList(rowData),
        ];
  ...
  /// ⚡ Bolt: Agrega multiples filas de asistencia en una sola llamada de red (Batch Insert)
